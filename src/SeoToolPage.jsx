          ...
          {result && (
            <>
              <div className="seo-result-head">
                ...
                <div className="seo-meta"><span>{result.generation_method === 'gemini-ai' ? 'Gemini 即時 AI' : result.execution_path === 'vercel-emergency' ? 'Vercel 緊急演示' : 'n8n 輸入驅動演示'}</span><span>{result.revised.char_count} 字</span><span>{result.revised.images.length} 張圖</span><span className={result.quality_gate === 'PASS' ? 'pass' : 'review'}>{result.quality_gate}</span><span>{result.folder_path}</span></div>
